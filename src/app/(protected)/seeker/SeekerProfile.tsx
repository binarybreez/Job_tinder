...
const SeekerProfile = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // First, get the response text to handle both JSON and text responses
      ...
      try {
        ...
        if (!response.ok) {
          // If it's an error response but not JSON, use the raw text as error message
          throw new Error(responseText || "Upload failed with unknown error");
        } else {
          // If it's a successful response but not JSON, this is unexpected
